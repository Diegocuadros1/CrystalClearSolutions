...
router.post(
  ...
  [
    ...
    check("time", "time is required").isTime(),
  ...
  async (req, res) => {
    ...
    //pulling out inputs
    const { name, email, date, time, address, zip, message } = req.body;

    const formData = {};

    if (name) formData.name = name;
    if (email) formData.email = email;
    if (date) formData.message = date;
    if (time) formData.message = time;
    if (address) formData.address = address;
    if (zip) formData.address = zip;
    if (message) formData.message = message;
    ...
    try {
      ...
      const mailOptions = {
        from: email,
        to: ["cuadrosda21@gmail.com", "diego@crystalclearsolutions.co"],
        subject: `${name} HAS BOOKED A DATE`,
        html: `<h3>Address: ${address}</h3>
                <p> Name: ${name} </p>
                <p> Email: ${email}</p>
                <p> Date: ${date}</p>
